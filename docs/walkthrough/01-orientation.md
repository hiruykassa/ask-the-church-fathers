...
- **The backend start command** lives in the container's `Dockerfile` `CMD` (`backend/Dockerfile`): `./prestart.sh && gunicorn -w 1 --threads 8 -b 0.0.0.0:${PORT:-5001} --timeout 60 app:app`. Note `-w 1`: **one** worker process, so the big embedding matrix lives in RAM exactly once. `--threads 8` gives that worker concurrency, since requests spend most of their time blocked on Gemini/Voyage and SQLite. App Runner injects `PORT`. (Historical note: the old `render.yaml:21` already used `-w 1 --threads 8`. The AWS Dockerfile dropped `--threads` during the migration, silently serializing every request; it was restored on 2026-07-30.)
