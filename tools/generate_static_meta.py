#!/usr/bin/env python3
"""Emit per-route static HTML with correct <head> meta, from database.db.

Phase 1 of docs/seo-static-meta-design.md (option B: directory-index files
plus a CloudFront viewer-request function).
...
def work_routes(conn: sqlite3.Connection, site: str) -> list[tuple[str, dict]]:
    ...
    out = []
    for wid, title, aid, author, category in rows:
