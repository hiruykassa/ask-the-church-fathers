...
| ~~**Sentry is wired but inactive**~~ | **Resolved 2026-07-31.** `SENTRY_DSN` is set on the App Runner service and the instance logs `Sentry error monitoring enabled` on boot. Errors only — `traces_sample_rate=0.0`, `send_default_pii=False`. The `environment` tag follows `PRODUCTION`, so an event arriving tagged `development` would mean `PRODUCTION=1` was dropped from the service config | — |
...
| ~~**Old hosting not confirmed cancelled**~~ | **Resolved 2026-07-31.** Render, Netlify, and Cloudflare R2 services and buckets were deleted and all three accounts confirmed on free tiers. Note for anyone repeating this: on Render and Netlify the *workspace/team subscription* bills separately from the services, so deleting services alone does not stop charges | — |
| **No automated deploys** | **Pushing to GitHub does not update the site.** `.github/workflows/ci.yml` has exactly two jobs — `backend` (pytest) and `frontend` (lint + build). Neither touches AWS. Every ship is a manual sequence of build, push to ECR, `start-deployment`, sync, invalidate, run by a human. Easy to forget the CloudFront invalidation, to build with the wrong `VITE_API_URL`, or to assume a green CI run means the change is live | A GitHub Actions deploy job gated on CI, using an OIDC role |
